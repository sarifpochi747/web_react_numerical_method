...
const MatrixCellJacobi= ({name,dataA,reset})=>{
    ...
    const [lableX,setLabelX] = useState("")
 ...
 //handlerChange inputmatrix A
    ...
    const handleChangeMatrixX = (row,event)=>{
        let copyX = matrix(X);
        copyX.set([row],event.target.value);
        setX(matrix(copyX));
    }

    const sendData = ()=>{
        dataA(A,B,X,matrixB.length);
    }
    ...
    const btnSubmit = (n)=>{
        ...
        setMatrixX(Array.from({length: n},()=>null));

        setLabelA("MATRIX A")
        setLabelB("MATRIX B")
        setLabelX("Set Defualt X0");
        ...
        <div>
            <div className="input-display-matrix">
                <label className="label-input-dimension">
                    Matrix dimension:
                    <input type={"number"} className="input-dimention" name="dimention"  onChange={(event)=>{
                        btnSubmit(event.target.value)
                    }} />
                </label>
                <div className="btn-dislpay-set-matrix">
                    <button type="button" disabled={btnStatus}  className="btn-set-matrix" onClick={btnSubmit}>SET MATRIX</button>
            ...
            <div className="display-matrix-cell">
                ...
                {/*Table Matrix B */}
                <div className="Defualt x0">{lableX}
                <Table className="table-matrixX">
                    <tbody className="tbody-matrixX">
                        {matrixX.map((r,rowIndex)=>{
                            return <tr className="tr-matrixX" key={rowIndex}>
                                <td className="td-matrix-x">
                                    <input className="input-matrixX"
                                        type={"number"}
                                        onChange={e=> handleChangeMatrixX(rowIndex,e)}
                                        />
                                </td>
                            </tr>
                        })}
...
export default MatrixCellJacobi;